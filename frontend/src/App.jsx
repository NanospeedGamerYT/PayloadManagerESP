...
function App() {
  ...
  const handleAbort = async () => {
    ...
    addToast("Secuencia abortada", "error")
  }
  ...
  const handleDelete = (fileName) => {
    showConfirm(
      ...
      async () => {
        ...
        if (!res.ok) {
          addToast(`Fallo al eliminar (${res.status})`, 'error')
          return
        }
        refreshPayloads()
  ...
  const handleUpload = async (e) => {
    ...
    if (ext !== 'elf' && ext !== 'bin') {
      addToast("Tipo de archivo no soportado. Solo se permiten archivos .elf y .bin.", "error");
      return;
    }
    ...
    try {
      ...
      if (data.file_exists || data.folder_exists) {
        setConfirmModal({
          show: true,
          title: "Sobrescribir Payload",
          message: data.file_exists
            ? `El archivo ${file.name} ya existe. ¿Quieres sobrescribirlo?`
            : `Una versión diferente de este payload ya existe en la carpeta "${data.folder_name}". ¿Quieres sobrescribirla?`,
  ...
  const performUpload = async (file) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Fallo al subir (${res.status})`)
      setDownloadModal(prev => ({ ...prev, progress: 100 }))
      addToast(`${file.name} subido correctamente`)
      refreshPayloads()
    } catch (e) { addToast(e.message || "Fallo al subir", "error") }
  ...
  const handleInstall = async (p, sourceId, repoUrl) => {
    if (p.isUpdate || p.isInstalled) {
      setConfirmModal({
        show: true,
        title: p.isUpdate ? "Actualizar Payload" : "Reinstalar Payload",
        message: `Ya hay una versión de ${p.name || p.filename} instalada. ¿Quieres reemplazarla con la versión del repositorio?`,
  ...
  const performInstall = async (p, sourceId, repoUrl) => {
    ...
    try {
      ...
      if (res.ok && data?.ok) {
        setDownloadModal(prev => ({ ...prev, progress: 100 }))
        addToast(`${p.filename} instalado con éxito`)
        refreshPayloads()
      } else throw new Error(data?.message || 'Fallo en la instalación')
    } catch (e) { addToast(e.message || 'Fallo en la instalación', 'error') }
    setTimeout(() => setDownloadModal({ show: false }), 800)
  }
  ...
  const handleSaveConfig = async (newConfig) => {
    ...
    if (success) {
      ...
      addToast("Fallo al guardar la configuración", "error")
      return false
    }
  }
    ...
    return (
      <div className="min-h-screen ps5-bg text-zinc-100 font-ps5 flex flex-col items-center justify-center p-4 text-center">
        <div className="max-w-lg p-12 bg-black/40 rounded-3xl border border-white/5">
          ...
          <h1 className="text-2xl font-bold mb-4 text-zinc-300">Payload Manager no se está ejecutando...</h1>
          <p className="text-lg text-zinc-400 leading-relaxed">Por favor, asegúrate de haber cargado el archivo <strong>pldmgr.elf</strong> en tu PS5 antes de iniciar esta aplicación.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    ...
    <div className={cn(
      ...
      <nav className={cn(
        ...
        <NavButton active={view === 'dashboard'} onClick={() => setView('dashboard')} icon={LayoutDashboard} label="Panel Principal" mobileLabel="INICIO" />
        <NavButton showSeparator active={view === 'storage'} onClick={() => setView('storage')} icon={Database} label="Gestionar Payloads" mobileLabel="GESTIÓN" />
        <NavButton showSeparator active={view === 'autoload'} onClick={() => setView('autoload')} icon={RefreshCw} label="Autocarga" mobileLabel="AUTO" />
        <NavButton showSeparator active={view === 'processes'} onClick={() => setView('processes')} icon={Cpu} label="Procesos Activos" mobileLabel="PROCESOS" />
        <NavButton showSeparator active={view === 'settings'} onClick={() => setView('settings')} icon={Settings} label="Ajustes" mobileLabel="AJUSTES" />
        <NavButton
          showSeparator
          active={view === 'donate'}
          onClick={() => setView('donate')}
                ...
                {loadingPayloads ? (
                  ...
                  <div className="col-span-full py-20 border-2 border-dashed border-white/5 rounded-ps-xl flex flex-col items-center justify-center space-y-6 bg-white/[0.01]">
                    ...
                    <div className="text-center">
                      <p className="text-white font-extrabold tracking-tight text-2xl">Biblioteca Vacía</p>
                      <p className="text-zinc-500 font-medium">Añade payloads desde el Repositorio en la Nube para comenzar.</p>
                    </div>
                    <button onClick={() => { setStorageScrollTarget('cloud-repository'); setView('storage'); }} className="px-8 py-3 bg-ps-blue text-white rounded-xl font-bold tracking-tight">Abrir Repositorio</button>
                  </div>
                ) : (
      ...
      {loading && (
        <div className="fixed inset-0 bg-ps-black/95 z-[9999] flex flex-col items-center justify-center space-y-12">
          ...
          <div className="text-center">
            <h4 className="text-4xl font-extrabold text-white tracking-tight mb-4 uppercase italic">{activeLoadingName || "Iniciando Núcleo"}</h4>
        ...
        <div className="fixed inset-0 z-[9999] bg-[#08080a] flex flex-col animate-in fade-in duration-300">
          <div className="p-6 md:p-8 border-b border-white/10 flex items-center justify-between bg-[#08080a]/95 backdrop-blur-xl sticky top-0 z-10">
            <div className="flex items-center space-x-4">
              ...
              <h3 className="text-3xl font-black text-white uppercase italic tracking-tighter">Registros (Logs)</h3>
            </div>
            <button
